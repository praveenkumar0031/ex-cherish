...
function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Authentication failed. Verify coordinates.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] flex items-center justify-center p-6 relative overflow-hidden">
        ...
        <div className="p-12 md:p-16">
            ...
            <form onSubmit={handleLogin} className="space-y-6">
                ...
                <button 
                    type="submit"
                    disabled={loading}
                    className="w-full bg-slate-900 hover:bg-blue-600 text-white font-black py-6 rounded-[2.5rem] shadow-2xl shadow-slate-900/20 transition-all flex items-center justify-center gap-4 uppercase tracking-[0.3em] text-[11px] active:scale-[0.98] disabled:bg-slate-200"
                >
                    {loading ? (
                        <>
      ...
      <div className="fixed bottom-10 left-10 text-white/10 select-none">
          <p className="text-[120px] font-[900] tracking-tighter leading-none">EXCHERISH</p>
          <p className="text-[20px] font-black tracking-[0.5em] ml-2">DISTRIBUTED OPERATING SYSTEM</p>
      </div>
    </div>
  );
}
